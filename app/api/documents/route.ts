...
async function overGlobalCap(): Promise<boolean | "unknown"> {
  try {
    ...
    const recent = (await store().recent(GLOBAL_HOURLY_CAP + 1)).filter(
...
/** 어르신이 사진을 올린다. 계정은 없다 — 보호자가 준 초대 링크의 토큰이 신원이다.
 *  job 을 만들고 **즉시** 반환한다. 완료를 기다리면 서버리스 함수가 최대 26초를 붙잡는다. */
export async function POST(req: Request) {
  ...
  // 초대 토큰 → 가구. 토큰이 없거나 틀리면 누구의 우편물인지 알 수 없다.
  const hField = form.get("h");
  const token = (typeof hField === "string" && /^[A-Za-z0-9_-]{20,64}$/.test(hField) ? hField : null) ?? readElderToken(req);
  if (!token) return json({ error: "보호자가 보낸 링크로 열어 주세요" }, 401);
  const db = store();
  const guardian = await db.guardianByElderToken(token).catch(() => null);
  if (!guardian) return json({ error: "링크가 더 이상 유효하지 않습니다. 보호자께 새 링크를 받아 주세요" }, 401);
